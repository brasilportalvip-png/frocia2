...
export interface ReverseCreditPurchaseParams {
  userId: string;
  paymentDocumentId: string;
  providerPaymentId: string | null;
...
export interface ReverseCreditPurchaseResult {
  success: true;
  availableAfter: number;
...
const walletRestricted =
  data.walletRestricted === true ||
  creditDebt > 0;

if (walletRestricted) {
  throw new InsufficientCreditsError(
    'Carteira temporariamente bloqueada devido a créditos estornados ou contestados.'
  );
}
...
if (availableBefore < amount) {
  ...
  /**
 * Reverses credits from a refunded or charged-back purchase.
 * Never creates a negative balance and is fully idempotent.
 */
static async reverseCreditPurchase(
  params: ReverseCreditPurchaseParams
): Promise<ReverseCreditPurchaseResult> {
  const {
    userId,
    paymentDocumentId,
    providerPaymentId,
  ...
  const txDocId = hashKey(
    'wallet:purchase-reversal',
    idempotencyKey
  );

  const payloadHash = hashKey(
    'payload:purchase-reversal',
    JSON.stringify({
      userId,
      paymentDocumentId,
      providerPaymentId,
  ...
  return adminDb.runTransaction(
    async (transaction) => {
      ...
      if (txSnap.exists) {
        ...
        if (
          ...
          throw new IdempotencyConflictError(
            'Chave de idempotência reutilizada com dados de estorno diferentes.'
          );
        }
        ...
        return {
          ...
          reversedCredits: Number(
            existing.reversedCredits ?? 0
          ),
          outstandingCredits: Number(
            existing.outstandingCredits ?? 0
          ),
          walletRestricted:
            existing.walletRestricted ===
            true
        };
      }
      ...
      if (!paymentSnap.exists) {
        throw new Error(
          `Pagamento ${paymentDocumentId} não encontrado para estorno.`
        );
      }
      ...
      if (
        ...
        throw new Error(
          'O pagamento não pertence ao usuário informado.'
        );
      }
      ...
      if (
        ...
        throw new Error(
          'O identificador do pagamento não corresponde ao registro interno.'
        );
      }

      const totalCredits = Number(
        paymentData.totalCredits ??
        (
          Number(
            paymentData.baseCredits ?? 0
          ) +
          Number(
      ...
      if (
        ...
        throw new InvalidAmountError(
          'O pagamento não possui uma quantidade válida de créditos para estorno.'
        );
      }
      ...
      if (!userSnap.exists) {
        throw new Error(
          'Usuário do pagamento não encontrado para estorno.'
        );
      }
      ...
      const availableBefore = Number(
        userData.creditsAvailable ??
        userData.creditsRemaining ??
        0
      );

      const reservedBefore = Number(
        userData.creditsReserved ?? 0
      );

      const purchasedBefore = Number(
        userData.creditsPurchased ?? 0
      );

      const consumedBefore = Number(
        userData.creditsConsumed ?? 0
      );

      const refundedBefore = Number(
        userData.creditsRefunded ?? 0
      );

      const creditDebtBefore = Number(
        userData.creditDebt ?? 0
      );

      if (
        paymentData.refundedCredits ===
        true
      ) {
        return {
          success: true,
          availableAfter:
            availableBefore,
          reversedCredits: Number(
            paymentData.reversedCredits ??
            0
          ),
          outstandingCredits: Number(
            paymentData.outstandingCredits ??
            0
          ),
          walletRestricted:
            userData.walletRestricted ===
              true ||
            creditDebtBefore > 0
        };
      }
      ...
      transaction.set(txRef, {
        ...
        operation:
          reason === 'refund'
            ? 'Estorno de créditos por reembolso do pagamento'
            : 'Estorno de créditos por contestação do pagamento',
        payloadHash,
        status: 'confirmed',
        idempotencyKey,
        createdAt:
